...
import { useNetworkStats, useLeaderboard } from '@/lib/queries/useStats'
import { mockDistrictSensorCounts } from '@/lib/mock/stats'
...
// Derived stats
const treeStatusCounts = {
  dry: mockTrees.filter((t) => t.humidity_status === 'dry').length,
  low: mockTrees.filter((t) => t.humidity_status === 'low').length,
  normal: mockTrees.filter((t) => t.humidity_status === 'normal').length,
  moist: mockTrees.filter((t) => t.humidity_status === 'moist').length,
}
const TREE_STATUS_PIE = [
  { name: 'Dry', value: treeStatusCounts.dry, color: '#ef4444' },
  { name: 'Low', value: treeStatusCounts.low, color: '#f97316' },
  { name: 'Normal', value: treeStatusCounts.normal, color: '#22c55e' },
  { name: 'Moist', value: treeStatusCounts.moist, color: '#06b6d4' },
]
const activeSensors = mockSensors.filter((s) => s.status === 'active').length
const SENSOR_STATUS_PIE = [
  { name: 'Active', value: activeSensors, color: '#06b6d4' },
  { name: 'Inactive', value: mockSensors.length - activeSensors, color: '#374151' },
]
const modelCounts = mockSensors.reduce<Record<string, number>>((acc, s) => {
  acc[s.model_type] = (acc[s.model_type] ?? 0) + 1
  return acc
}, {})
const MODEL_DATA = Object.entries(modelCounts).map(([name, value]) => ({ name, value }))
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            { label: 'Registered Trees', value: stats?.registered_trees ?? 0, icon: TreePine, color: '#22c55e' },
            {
              ...
              value: `${Math.round((activeSensors / mockSensors.length) * 100)}%`,
              icon: Activity,
              color: '#06b6d4',
            },
            { label: 'Districts Covered', value: mockDistrictSensorCounts.filter((d) => d.sensor_count > 0).length, icon: MapPin, color: '#22c55e' },
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className={CHART_CARD}>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={MODEL_DATA} layout="vertical" margin={{ left: 0 }}>
          ...
          <div className={CHART_CARD}>
            ...
            <div className="relative flex items-center justify-center" style={{ height: 180 }}>
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={SENSOR_STATUS_PIE} cx="50%" cy="50%" innerRadius="55%" outerRadius="75%" dataKey="value" startAngle={90} endAngle={-270}>
                    {SENSOR_STATUS_PIE.map((entry, i) => (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-2xl font-bold text-white">{Math.round((activeSensors / mockSensors.length) * 100)}%</span>
                <span className="text-xs text-gray-400">Active</span>
              </div>
            </div>
            <div className="flex justify-center gap-4 mt-2">
              {SENSOR_STATUS_PIE.map((e) => (
          ...
          <div className={CHART_CARD}>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie data={TREE_STATUS_PIE} cx="50%" cy="50%" outerRadius="70%" dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={false}>
                  {TREE_STATUS_PIE.map((entry, i) => (
                    <Cell key={i} fill={entry.color} />
                  ))}
                </Pie>
                <Tooltip content={<ChartTooltipStyle />} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className={CHART_CARD}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={mockDistrictSensorCounts} layout="vertical" margin={{ left: 0 }}>
                <CartesianGrid {...GRID_COLORS} strokeDasharray="3 3" horizontal={false} />
                <XAxis type="number" tick={AXIS_TICK} tickLine={false} axisLine={false} />
